...
class Pooling(Module):
    ...
    def execute(self, features: dict[str, jt.Var]) -> dict[str, jt.Var]:
        token_embeddings = features["token_embeddings"]
        attention_mask = (
            features["attention_mask"]
            if "attention_mask" in features
            else jt.ones(token_embeddings.shape[:-1], dtype=jt.float32)
        )
        ...
        if self.pooling_mode_max_tokens:
            ...
            max_over_time = jt.max(token_embeddings, 1)[0]
            output_vectors.append(max_over_time)
        if self.pooling_mode_mean_tokens or self.pooling_mode_mean_sqrt_len_tokens:
            input_mask_expanded = (
                attention_mask.unsqueeze(-1).expand(token_embeddings.size())
            )
            sum_embeddings = jt.sum(token_embeddings * input_mask_expanded, 1)
            ...
            if self.pooling_mode_mean_sqrt_len_tokens:
                output_vectors.append(sum_embeddings / jt.sqrt(sum_mask))
        if self.pooling_mode_weightedmean_tokens:
            input_mask_expanded = (
                attention_mask.unsqueeze(-1).expand(token_embeddings.size()).to(token_embeddings.dtype)
            )
            ...
            weights = (
                jt.arange(start=1, end=token_embeddings.shape[1] + 1)
                .unsqueeze(0)
                .unsqueeze(-1)
            ...
            sum_mask = jt.clamp(sum_mask, min=1e-9)
            output_vectors.append(sum_embeddings / sum_mask)
        if self.pooling_mode_lasttoken:
            ...
            if jt.jit.is_tracing():
                ...
                attention_mask = attention_mask.to(jt.float32)

            values, indices = attention_mask.flip(1).max(1)
            indices = jt.where(values == 0, seq_len - 1, indices)
            ...
            embedding = jt.gather(token_embeddings * input_mask_expanded, 1, gather_indices).squeeze(dim=1)
            output_vectors.append(embedding)

        output_vector = jt.cat(output_vectors, 1)
